...
/**
 * Thread Auditor class: - Provides a mechanism for applications to check the
 * health of internal threads - The mechanism is fairly simple: - Threads
 * register with the auditor at startup and "ping" the auditor every so often. -
 * The application queries the auditor about the health of the system
 * periodically. The auditor reports if the threads are healthy or if any of
 * them failed to ping and are considered dead or stuck. - The main implication
 * for the monitored threads is that they can no longer block waiting for an
 * event forever. Any wait() must be implemented with a timeout so that the
 * thread can periodically ping the auditor.
 * 
...
public class ThreadAuditor {
	...
	public class ThreadHandle {
		// / Set to true when the thread pings, periodically reset to false by
		// the auditor
		...
		// Helper function to allow threads to get the ping interval directly
		// from this handle
		...
		public String toString() {
			StringBuffer toString = new StringBuffer().append("Thread Name: ")
					.append(thread.getName()).append(", Alive: ").append(
							thread.isAlive());
			return toString.toString();
		}
	}

	// / Indicates how often monitored threads are supposed to ping (0 = no
	// thread monitoring)
	public long getPingIntervalInMillisecs() {
	...
	public synchronized ThreadHandle addCurrentThread() {
		...
		ThreadHandle threadHandle = this.threadHandles.get(Thread
				.currentThread());
	...
	public synchronized String auditThreads() {
		...
		while (it.hasNext()) {
			...
			if (!threadHandle.isThreadActive()) {
				...
				auditReport += "   Thread [" + thread.getName()
						+ "] has failed to respond to an audit request.\n";

				/*
				 * Stack traces are not available with JDK 1.4. Feel free to
				 * uncomment this block to get a better report if you're using
				 * JDK 1.5.
				 */
				// // Get stack traces for all live threads (do this only once
				// per audit)
				// if (stackTraces == null) {
				...
				// if (stackTraceElements != null && stackTraceElements.length >
				// 0) {
