 ...
 * 
 * 
 * @version 1.2 $Revision: 1.44.4.1 $ $Date: 2007-11-21 23:55:34 $
 */
public class UDPMessageChannel extends MessageChannel implements
	...
	protected UDPMessageChannel(SIPTransactionStack stack,
			UDPMessageProcessor messageProcessor, byte[] bytesRead, InetAddress peerAddress, int port) {

		this.bytesRead = bytesRead;
		this.peerPacketSourceAddress = peerAddress;
		this.peerPacketSourcePort = port;
	...
	public void run() {
		...
		// We're part of a thread pool. Ask the auditor to monitor this
		// thread.
		...
		try {
			processIncomingDataPacket(this.bytesRead);
		} catch (Exception e) {
			e.printStackTrace();
	 ...
	 *            is the incoming datagram packet.
	 */
	private void processIncomingDataPacket(byte[] msgBytes)
			throws Exception {
		int packetLength = msgBytes.length;
	...
	protected void sendMessage(byte[] msg, InetAddress peerAddress,
		...
		try {
			...
			if (sipStack.udpFlag) {
				...
				IoSession ioSession = ((UDPMessageProcessor) messageProcessor)
						.getSession();
				if (ioSession == null) {
					DatagramConnector datagramConnector = new DatagramConnector();
					ConnectFuture future = datagramConnector.connect(
				...
				((UDPMessageProcessor) messageProcessor).getSession()
						.write(byteBuffer);
		...
		if (peerProtocol.compareToIgnoreCase("UDP") == 0) {
			...
			try {
				if (sipStack.udpFlag) {
					...
					InetSocketAddress remoteAddr = (InetSocketAddress) ioSession
							.getRemoteAddress();
					ByteBuffer byteBuffer = ByteBuffer.wrap(msg);
					if (remoteAddr.getAddress().equals(peerAddress)) {
						((UDPMessageProcessor) messageProcessor).getSession()
								.write(byteBuffer);
					} else {
						DatagramConnector datagramConnector = new DatagramConnector();
