	...
	public void processBye(
		...
		try {
			...
			if (serverTransactionId == null) {
			    serverTransactionId = udpProvider.getNewServerTransaction(request);
...
*/
	...
	public void processResponse(ResponseEvent responseReceivedEvent) {
		...
		try {
			if (response.getStatusCode() == Response.OK
				...
				// ct.sendRequest();
				Request ackRequest = tid.createAck();
				System.out.println("Sending ACK");
				
				udpProvider.sendRequest(ackRequest);
				...
				if (reInviteCount == 0) {
					...
					(Request)tid.getRequest().clone();
				    ToHeader toHdr = 	(ToHeader) inviteRequest.getHeader(ToHeader.NAME);
				    toHdr.setTag(
				    ...
				    int seqno = cseq.getSequenceNumber();
				    // next in the dialog.
				    cseq.setSequenceNumber(seqno+1);
	...
	public void init() {
		...
		// stack manages dialogs unless this property is set.
