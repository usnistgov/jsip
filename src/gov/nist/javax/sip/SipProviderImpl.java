...
public final class SipProviderImpl
	 ...
	 * the SipProvider.
	 *
	 * @param sipListener SipListener to be registered with the
	 * Provider.
	...
	public CallIdHeader getNewCallId() {
		String callId =
			Utils.generateCallIdentifier(this.getSipStack().getIPAddress());
	...
	public ClientTransaction getNewClientTransaction(Request request)
		...
		if (request.getMethod().equalsIgnoreCase(Request.CANCEL)) {
			...
			if (ct != null) {
				ClientTransaction retval =
					sipStack.createClientTransaction(ct.getMessageChannel());
				((SIPTransaction) retval).setOriginalRequest(
					(SIPRequest) request);
			...
			while (it.hasNext()) {
				...
				try {
					...
					if (sipRequest.getTopmostVia() != null) {
						...
						Via via =
							this.listeningPoint.messageProcessor.getViaHeader();
						sipRequest.addHeader(via);
		...
		if (LogWriter.needsLogging) {
			sipStackImpl.logMessage("Error processing " + sipRequest);
		}
	 ...
	 *
	 * @param request The initial Request message that the doesn't match an existing
	...
	public ServerTransaction getNewServerTransaction(Request request)
		...
		if (sipStack.isDialogCreated(sipRequest.getMethod())) {
			...
			transaction =
				(SIPServerTransaction) sipStack.findTransaction(
					(SIPRequest) request,
					true);
			if (transaction != null)
	 ...
	 * listening point.
	 *
	 * @param listeningPoint the <var>listeningPoint</var> of this SipProvider
	 * @see ListeningPoint
	 * @since v1.1
	 */
		...
		if (transactionErrorEvent.getErrorID()
			...
			if (errorObject instanceof SIPServerTransaction) {
				ev =
					new TimeoutEvent(
						this,
						(ServerTransaction) errorObject,
						timeout);
			} else {
				ev =
					new TimeoutEvent(
						this,
						(ClientTransaction) errorObject,
						timeout);
			}
			// Handling transport error like timeout
			this.handleEvent(ev, (SIPTransaction) errorObject);
		} else if (
			transactionErrorEvent.getErrorID()
