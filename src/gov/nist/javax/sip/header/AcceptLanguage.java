...
public class AcceptLanguage
	extends ParametersHeader implements AcceptLanguageHeader {
    ...
    if (q < 0.0 || q > 1.0) 
			throw new InvalidArgumentException
				("qvalue out of range!");
	if (q == -1) this.removeParameter("q");
	else this.setParameter(new NameValue("q",new Float(q)));
    }
	...
	if (this.languageRange == null) return null;
	else 
	return new Locale(this.languageRange,Locale.getDefault().getCountry() );
    }
