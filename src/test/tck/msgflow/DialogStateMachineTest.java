...
public class DialogStateMachineTest extends MessageFlowHarness
{
    ...
    public void testClientDialogStates()
    {
        try
        {
            ...
            if(inviteReqEvt==null
               ||inviteReqEvt.getRequest()==null)
            ...
            assertNull(
                "A dialog passed into the "+dialog.getState()+" state before recieving any response!",
                dialog.getState()
                );
            ...
            if(ringingRespEvt==null
               ||ringingRespEvt.getResponse()==null)
                throw new TiUnexpectedError("The TI did not pass RINGING response to the TU.");
            //The dialog should now be in its early state.
            assertEquals("The Dialog did not pass into the early state upon reception of a RINGING response",
                         DialogState.EARLY,
                         dialog.getState());
            ...
            if(okRespEvt==null
               ||okRespEvt.getResponse()==null)
                throw new TiUnexpectedError("The TI did not pass OK response to the TU.");
            //The dialog should now be in its confirmed state.
            assertEquals("The Dialog did not pass into the CONFIRMED state upon reception of an OK response",
            ...
            assertTrue("The dialog did not pass into a final (COMPLETED or TERMINATED) state after sending a BYE.",
    ...
    public void testServerDialogStates()
    {
		...
		inviteTransaction.sendRequest();
            ...
            if(inviteReqEvt==null
               ||inviteReqEvt.getRequest()==null)
                throw new TiUnexpectedError(
                    "The TI did not dispatch the initial invite request");
            ...
            //We should have a null state here
            assertNull(
                "A dialog passed into the "+dialog.getState()
            ...
            catch(TooManyListenersException e)
            {
                throw new TckInternalError(
                    "Failed to register a SipListener with the RI.", e);
            }
            Response ringing = null;
            try
            {
...
//!discuss with Ranga
            ...
            waitForMessage();
            ResponseEvent ringingRespEvt = eventCollector.
                                           extractCollectedResponseEvent();
            if(ringingRespEvt==null
               ||ringingRespEvt.getResponse()==null)
                throw new TiUnexpectedError(
                    "The TI did not send the RINGING response.");
            //The dialog should now be in its early state.
            assertEquals("The Dialog did not pass into the early state after sending a RINGING response",
                         DialogState.EARLY,
                         dialog.getState());
            ...
            try
            {
                ok = tiMessageFactory.createResponse(Response.OK,
                                                     inviteReqEvt.getRequest());
		ContactHeader contact = createTiContact();
            ...
            if(okRespEvt==null
               ||okRespEvt.getResponse()==null)
                throw new TiUnexpectedError(
                    "The TI did not send an OK response.");
            //The dialog should now be in its CONFIRMED state.
            assertEquals("The Dialog did not pass into the CONFIRMED state upon reception of an OK response",
                         DialogState.CONFIRMED,
                         dialog.getState());
		...
		d.sendRequest(ct);
            ...
            {
                throw new TckInternalError(
                    "Failed to create and send a BYE request using a dialog.", e);
            }
            waitForMessage();
            RequestEvent byeEvt = eventCollector.extractCollectedRequestEvent();
            if(byeEvt==null
               ||byeEvt.getRequest()==null)
...
**/
	    ...
	    // Dialog enters a COMPLETED/TERMINATED state
            ...
            if(byeOkRespEvt==null
               ||byeOkRespEvt.getResponse()==null)
                throw new TiUnexpectedError(
                    "The TI did not send an OK response to a bye request.");
            //The dialog should now be in the COMPLETED/TERMINATED state.
            assertTrue("The dialog did not pass into a final (COMPLETED or TERMINATED) state after recieving a BYE.",
                       DialogState.COMPLETED.equals(dialog.getState())
                       ||DialogState.TERMINATED.equals(dialog.getState())
                       );
        }catch(Throwable exc)
