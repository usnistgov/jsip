...
public class NonInviteClientTransactionsStateMachineTest extends MessageFlowHarness
{
    ...
    public void testTryingProceedingCompletedTerminatedScenario()
    {
        try
        {
            ...
            if(registerReceivedEvent==null
               ||registerReceivedEvent.getRequest()==null)
                throw new TiUnexpectedError(
                    "The REGISTER request was not received by the RI!");
            ...
            if(tran.getRetransmitTimer()>MESSAGES_ARRIVE_FOR)
                sleep(
                    ...
                    -MESSAGES_ARRIVE_FOR); //subtract the time we waited for the REGISTER
            ...
            try
            {
               Response res =        riMessageFactory.createResponse(
		...
		addStatus(registerReceivedEvent.getRequest(),res);
            ...
            try
            {
                ok = riMessageFactory.createResponse(
                    Response.OK, registerReceivedEvent.getRequest()
		);
		addStatus( registerReceivedEvent.getRequest() , ok);
            ...
            {
                throw new TckInternalError(
                    "The TCK could not send a OK response back to the TI", ex);
            }
            waitForMessage();
            ...
            assertNull("The TI sent an ACK request in a non INVITE transaction",
                       ackReceivedEvent);
            ...
            assertNull("The TI replied with an ACK to a nonINVITE request",
                       ackReceivedEvent);
        }
        catch(Throwable exc)
        {
    ...
    {
        junit.swingui.TestRunner.run( NonInviteClientTransactionsStateMachineTest.class );
    }
