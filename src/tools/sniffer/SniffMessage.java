...
public class SniffMessage implements ParseExceptionListener {
  ...
  private void getSipMessage(ArrayList sniffMsgList) throws ParseException {
    ...
    for (int j=sdpIndex ; j < sniffMsgList.size() ; j++) {
      ...
      if (line.indexOf("(") > 0 && line.indexOf(")") > 0) {
        msgBuffer = msgBuffer + line.charAt(line.indexOf(":")-2) + "=" +
  ...
  public String toXML() {
    ...
    xmlMessage += "<message from=\"" + sourceIP + "\" to=\"" + destIP +
	    	 "\" time=\"" + time +
	    	 "\" isSender=\"" + true +
		 "\" callId=\"" + sipMessage.getCallId().getCallId() +
	    	 "\" statusMessage=\"" + statusMessage +
	    	 "\" transactionId=\"" + sipMessage.getTransactionId() +
	    	 "\" firstLine=\"" + firstLine +
	    	 "\">\n";
    xmlMessage += "<![CDATA[";
    xmlMessage += message;
    ...
    System.out.println("Error line = " +  headerText);
     ...
     try {
     if (headerClass.equals(Class.forName("gov.nist.javax.sip.header.From") )
	|| headerClass.equals(Class.forName("gov.nist.javax.sip.header.To") )
	|| headerClass.equals (Class.forName("gov.nist.javax.sip.header.ViaList") )
	|| headerClass.equals(Class.forName("gov.nist.javax.sip.header.CSeq"))
	|| headerClass.equals(Class.forName("gov.nist.javax.sip.header.CallId"))
     ) throw ex;
