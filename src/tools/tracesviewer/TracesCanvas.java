...
public class TracesCanvas extends Canvas implements MouseListener , MouseMotionListener{
	...
	public TracesViewer tracesViewer;
        ...
        public TracesCanvas(TracesSession tracesSession,TextArea messageContentTextArea,
            String trueName,TracesViewer tracesViewer){
        ...
        // Reserved constructor for the AboutFrame
         public TracesCanvas(TracesSession tracesSession,Image backgroundImage,
            String trueName,int horizontalGap,TracesViewer tracesViewer){
	    this.tracesViewer=tracesViewer;
            ...
            while(e.hasMoreElements()){
              ...
              //       g.drawString(tracesOrigin,positionX - getFontMetrics(g.getFont()).stringWidth(tracesOrigin) / 2 ,
               ...
               // }
                ...
                g.setFont(newFont) ;
                g.drawString(origin,positionX - getFontMetrics(g.getFont()).stringWidth(origin) / 2 ,
                ACTORS_STRIPE - 15) ;
               
                if(actorsImage != null) {
                  
                    g.drawImage(actorsImage,positionX - actorsImage.getWidth(this) / 2 ,
                    ACTORS_STRIPE/3,this) ;
                }
                else {
		...
		for(int i = 0 ; i < tracesSession.size() ; i++){
			...
			// We have to stock the actor and its position in the canvas
				...
				actors.put(from,new Integer(sizeActors * HORIZONTAL_GAP + FIRST_ACTOR_GAP)) ;
			}
                        sizeActors=actors.size();
			if(actors.get(to) == null){
				actors.put(to,new Integer(sizeActors * HORIZONTAL_GAP + FIRST_ACTOR_GAP)) ;
			}
	    ...
	    assignColors() ;
            ...
            for(int i = 0 ; i < tracesSession.size() ; i++){
                ...
                if( positionXFrom== positionXTo) {
                    // This is a loop !!
                    CircleArrow circleArrow=new
                    CircleArrow(selected,arrowName,positionXFrom,positionY-20, positionY+20,40,true,info);
                    ...
                    StraightArrow straightArrow=new StraightArrow(selected,arrowName,
                     positionXFrom,positionXTo,positionY-31, positionY+31, true,info);	
        ...
        public void assignColors(){
            ...
            for(int i = 0 ; i < tracesSession.size() ; i++){
                ...
                if( transactionId != null) {
                    ...
                    if( color == null) {
                        if (colorIndex >=10) color=Color.black;
                        else color=ARROWS_COLOR[colorIndex];
                ...
                // big problem
                    ...
                    if (messageContentTextArea!=null) {
                        ...
                        if (tracesMessage.debugLine!=null && !tracesMessage.debugLine.equals("") ) {
                                ...
                                tracesViewer.messageContentButton.
                             setText("SIP message: (debug log line: "+tracesMessage.debugLine+" )");
                        }
                    }
