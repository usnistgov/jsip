...
public class InviteClientTransactionsStateMachineTest
	...
	public void testCallingProceedingCompletedTerminatedScenario() {
		try {
			...
			assertNotNull(
				"The TI did not send an ACK request",
				ackReceivedEvent);
			assertNotNull(
				"The TI did not send an ACK request",
			...
			assertEquals(
				Request.ACK,
				ackReceivedEvent.getRequest().getMethod());
			...
			if (ackReceivedEvent != null)
				System.err.println(
					"Shouldn't have received that="
						+ ackReceivedEvent.getRequest());
			assertNull(
				"The TI sent an ACK to an OK (this is TU's job)!",
				ackReceivedEvent);
	...
	public static void main(String[] args) {
		junit.swingui.TestRunner.run(
			InviteClientTransactionsStateMachineTest.class);
	}
