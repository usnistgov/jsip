...
public class TestHarness  extends TestCase {
   ...
   private static void println(String messageToPrint)  {
	...
	path = System.getProperties().getProperty
			(IMPLEMENTATION_PATH);
	String flag = System.getProperties().getProperty
			(ABORT_ON_FAIL);
	String lf = System.getProperties().getProperty
			(LOG_FILE_NAME);
	if (lf != null) logFileName = lf;
	abortOnFail =  ( flag != null && 
	  ...
	  try {
		...
		if (! logFile.exists()) {
			...
			printWriter = null;
		     ...
		     System.out.println("please remove the log file " + 
				logFileName);
		     System.exit(0);
		}
    ...
    private static void logFailure(String message, Object[] args) {
        ...
        if (args != null)  {
            for (int i = 0; i < args.length; i++) {
                ...
                if (i < args.length) stringBuffer.append(",");
            }
        }
        stringBuffer.append(")");
	...
	println
	 ( " Test point " + testCounter +  " in function " + 
		reason + " failed " );
	if (abortOnFail) {
