...
public class ClientTransactionTest extends MessageFlowHarness
{
        ...
        try
        {
            ...
            catch(TransactionUnavailableException exc)
            {
                throw new TiUnexpectedError(
                    ...
                    +"create a new client transaction", exc);
            }
            Request cancel = null;
            try
            {
            ...
            assertEquals("The created request did not have a CANCEL method.",
                         cancel.getMethod(), Request.CANCEL);
            assertEquals(
                "Request-URIs of the original and the cancel request do not match",
                cancel.getRequestURI(), invite.getRequestURI());
            assertEquals(
    ...
    public void testCreateAck()
    {
        try
        {
            ...
            assertEquals("The created request did not have a CANCEL method.",
                         ack.getMethod(), Request.ACK);
            assertEquals(
                "Request-URIs of the original and the ack request do not match",
                ack.getRequestURI(), invite.getRequestURI());
            assertEquals("Call-IDs of the original and the ack request do not match",
                         ack.getHeader(CallIdHeader.NAME),
                         invite.getHeader(CallIdHeader.NAME));
            assertEquals("ToHeaders of the original and the ack request do not match",
