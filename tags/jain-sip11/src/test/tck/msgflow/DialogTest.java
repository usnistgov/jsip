    ...
    extends MessageFlowHarness
{
    ...
    public void testDialogProperties()
    {
        ...
        {
            //CallId
            assertEquals("The Dialog did not have the right Call ID.",
                         ((CallIdHeader)riInvite.getHeader(CallIdHeader.NAME)).getCallId(),
            ...
            //LocalParty
            assertEquals("Dialog.getLocalParty() returned a bad address.",
                         ((FromHeader)tiInvite.getHeader(FromHeader.NAME)).getAddress(),
                         dialog.getLocalParty());
            //SeqNum
            assertTrue("Dialog.getLocalSequenceNumber() returned a bad value.",
                       1
                       ==dialog.getLocalSequenceNumber());
            //LocalTag
            assertEquals("Dialog.getLocalTag() returned a bad tag",
                         ((FromHeader)riInvite.getHeader(FromHeader.NAME)).getTag(),
                         dialog.getLocalTag());
            //RemoteParty
            assertEquals("Dialog.getRemoteParty() returned a bad address.",
                         ((ToHeader)tiInvite.getHeader(ToHeader.NAME)).getAddress(),
                         dialog.getRemoteParty());
            //RemoteTag
            assertEquals("Dialog.getRemoteTag() returned a bad tag",
    ...
    public void testCreateRequest()
    {
        try
        {
            ...
            assertEquals(
                ...
                ((CSeqHeader)bye.getHeader(CSeqHeader.NAME)).getSequenceNumber()
                );
            //Check From
            ...
            if(okRespEvt==null
               ||okRespEvt.getResponse()==null)
                throw new TiUnexpectedError(
                    "The TI did not dispatch an OK response.");
            //Send the ack
