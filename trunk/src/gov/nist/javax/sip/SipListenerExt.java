 ...
 * <ul>
 * <li>{@link DialogTimeoutEvent}- these are timeout notifications emitted as events
 * by the SipProvider. Timeout events represent timers expiring in the
 * underlying SipProvider dialog state machine. These timeout's events
 * notify the application that a dialog has timed out.</li>
 * </ul>
...
public interface SipListenerExt extends SipListener {
     ...
     * {@link Dialog} handled by this SipListener. This Event notifies the
     * application that a dialog Timer expired in the
     * Dialog's state machine. The DialogTimeoutEvent encapsulates
     * the specific timeout type and the dialog identifier. The type of Timeout can by
     * determined by:
     ...
     * 
     * @param timeoutEvent -
     *            the timeoutEvent received indicating the dialog timed out.
     */
