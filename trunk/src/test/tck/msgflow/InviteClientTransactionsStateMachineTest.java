...
public class InviteClientTransactionsStateMachineTest extends MessageFlowHarness
{
    ...
    {
        super(name,false);	// disable auto-dialog for the RI, else ACKs get filtered out
    }
    //==================== tests ==============================
    ...
    public void testCallingProceedingCompletedTerminatedScenario()
    {
        try
        {
            ...
            try
            {
                ...
                // JvB: set to-tag too, mandatory and tests that ACK is properly formatted
            ...
            assertEquals(
                ...
                Response.BUSY_HERE, responseEvent.getResponse().getStatusCode() );
            assertEquals(
            ...
            // JvB: With auto-dialog-support enabled, the ACK should be filtered by the RI            
    ...
    /**
     * JvB: Tests transmission of an INVITE followed by CANCELlation of that request
     *
     * -> INVITE
     * <- 100
     * -> CANCEL
     ...
     * 
    public void testInviteCancelNonRFC3261()
    {
    	doCancelTest(false);
    }
    */
     ...
     * -> ACK
     * 
     * Note: for 1.2 it is impossible to manually set the top via branch to something
     * not RFC3261-compliant
     */    	
    private void doCancelTest( boolean rfc3261Compliant ) {
        try
        {
            ...
            if (!rfc3261Compliant) {
            	((ViaHeader) tiCancel.getHeader("Via")).setBranch( "xxx" );
            	// Not allowed by RI
            	// ((FromHeader) tiCancel.getHeader("From")).setTag( "" );
            }*/
            ...
            {
                throw new TiUnexpectedError(
                    "A SipExceptionOccurred while trying to send CANCEL!", ex);
            }
            waitForMessage();
            ...
            //Analyze the REQUEST_TERMINATED response and Tran state back at the TI
            responseEvent = eventCollector.extractCollectedResponseEvent();
            assertNotNull(
                "The Tested Implementation did not pass a 300-699 response to the TU!",
                responseEvent);
    ...
    public void testCallingCompletedTerminatedScenario()
    {
        try
        {
            ...
            // JvB: test
            // ((MessageExt) invite).getFirstViaHeader().setParameter( "rport", null );
            ...
            // JvB: verify to tag
            assertEquals( "The To header field in the ACK MUST equal the To header field " +
...
/*
 * JvB: the stack should in fact ~not~ pass the retransmitted response, but filter 
 * it out
 *             
            assertNotNull(
...
*/             
            // TBD
            // assertNull( "The TI should filter the retransmitted OK response", responseEvent );    
