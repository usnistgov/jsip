...
public class TracesViewer extends javax.swing.JFrame {
    ...
    */
    ...
    public TracesViewer(String title, 
		String rmiHost, String rmiPort, 
		String stackId, 
...
/******************** THIRD SUB PANEL ****************************************/
    ...
    public static void usage(){
        ...
        "-f tracesfile") ;
        System.out.println("*************************************\n") ;
        System.out.println("When viewing from an RMI connection:"+
        "java tools.tracesviewer.tracesViewer " +
    ...
    public static void main(String args[]) {
        try{
			...
			new  TracesViewer("Traces viewer",
                "127.0.0.1","1099","127.0.0.1:5060");
                
            }
            else
            if(args[0].equals("-debug_file")){
		...
		return;
			...
			k++;
                     ...
                     if (args[k].equals("-rmiport")) {
			rmiPort = args[k+1] ;
			k++;
		     } 
                     else 
                     if (args[k].equals("-stackId")) {
			stackId = args[k+1] ;
			k++;
		     } 
                     else
                     if (args[k].equals("-back")) {
                        launcher=true;
			back = args[k+1] ;
			k++;
		     } 
                     else
                     if (args[k].equals("-faces")) {
			faces = args[k+1] ;
			k++;
		     } 
                     else
                     if (args[k].equals("-actors")) {
			actors = args[k+1] ;
			k++;
		     } 
                     else
                     if (args[k].equals("-logoNist")) {
			logoNist = args[k+1] ;
			k++;
		     } 
                     else usage();
		 ...
		 if (stackId == null)  {
			...
			System.exit(0);
                ...
                if (launcher ) 
                     tracesViewer=new TracesViewer("Traces viewer",
                rmiHost,rmiPort,stackId,back,faces,actors,logoNist);
                else
                    tracesViewer=new  TracesViewer("Traces viewer",
                rmiHost,rmiPort,stackId);
		System.out.println
		("Everything is Ok ... Launching the Traces viewer") ;
           }
       }
            ...
            System.out.println
	    ("Problem starting viewer. I give up :)");
            e.printStackTrace();
       }
   }
